  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs">
      <div className="w-full max-w-3xl rounded-2xl border border-neutral-200 dark:border-white/10 bg-white dark:bg-[#0d0d10] shadow-2xl overflow-hidden flex flex-col h-[600px]">
        ...
        <div className="p-5 border-b border-neutral-200 dark:border-white/10 flex items-center justify-between bg-neutral-50 dark:bg-[#111115]">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">RAG 层级 (KnowledgeBase → Source → Document → Chunk) 的真实数据管道</p>
            </div>
          </div>
        ...
        {/* Tab Header */}
        <div className="flex border-b border-white/10 bg-neutral-900 text-xs font-mono">
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 text-xs">
          {activeTab === "jobs" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="font-bold text-slate-300 font-mono">历史 Sync Job 执行日志 ({syncJobs.length})</span>
                {source && (
                  <button
              ...
              <div className="space-y-3">
                {syncJobs.map((job) => (
                  <div
                    key={job.id}
                    className="p-4 rounded-xl border border-white/10 bg-neutral-900/80 space-y-2"
                  >
                    <div className="flex items-center justify-between font-mono">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="font-bold text-slate-200">{job.sourceName}</span>
                      </div>
                    ...
                    <div className="flex items-center space-x-4 text-slate-400 font-mono text-[11px]">
                      <span>总数: <strong className="text-slate-200">{job.totalDocs}</strong></span>
                      <span>已切片: <strong className="text-emerald-400">{job.syncedDocs}</strong></span>
                      <span>失败: <strong className="text-red-400">{job.failedDocs}</strong></span>
                      <span>时间: {job.timestamp}</span>
                    </div>

                    {job.errorMessage && (
                      <div className="p-2.5 rounded bg-red-950/40 border border-red-500/30 text-red-300 font-mono text-[11px] leading-relaxed">
          ...
          {activeTab === "mapping" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-slate-200 text-sm">数据库源与向量 Chunk 字段对齐拓扑</h3>
                  <p className="text-slate-400 text-xs">配置 IPMS 关系数据库源的列对应到向量嵌入 (Embedding) 索引的对应 Target 字段</p>
                </div>

                <div className="flex items-center space-x-2">
                  <button
                    onClick={handleAddField}
                    className="px-3 py-1.5 rounded-lg border border-white/10 text-slate-300 hover:bg-white/5 font-mono flex items-center space-x-1"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold font-mono flex items-center space-x-1"
                  >
              ...
              <div className="rounded-xl border border-white/10 overflow-hidden bg-black font-mono">
                <table className="w-full text-left text-xs">
                  <thead className="bg-neutral-900 text-slate-400 border-b border-white/10">
                    <tr>
                      ...
                      <tr key={idx} className="hover:bg-white/5">
                        <td className="p-3">
                          <input
                            ...
                            className="bg-neutral-900 border border-white/10 rounded px-2 py-1 text-slate-100 font-mono w-full"
                          />
                        </td>
                        <td className="p-3">
                          <input
                            ...
                            className="bg-neutral-900 border border-white/10 rounded px-2 py-1 text-blue-300 font-mono w-full"
                          />
                        </td>
                        <td className="p-3">
                          <select
                            ...
                            className="bg-neutral-900 border border-white/10 rounded px-2 py-1 text-slate-300 font-mono"
                          >
