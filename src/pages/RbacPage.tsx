          ...
          return (
            <div
              ...
              className={`p-4 rounded-2xl border cursor-pointer space-y-2 transition-all ${
                isSelected
                  ? "border-emerald-500 bg-emerald-500/10 text-emerald-300 shadow-md font-bold"
                  : "border-neutral-200 dark:border-white/10 bg-white dark:bg-[#0d0d10] text-slate-400 hover:border-white/20"
              }`}
            >
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-100 font-bold">{role.name.split(" ")[0]}</span>
                {role.id === userRole && (
      ...
      <div className="p-6 rounded-2xl border border-neutral-200 dark:border-white/10 bg-white dark:bg-[#0c0c0e] shadow-xl space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs font-mono transition-colors shrink-0"
          >
            应用此角色作为试用身份
          </button>
        </div>
        ...
        {/* Matrix Table */}
        <div className="rounded-xl border border-white/10 overflow-hidden bg-black font-mono text-xs">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-white/10 text-slate-200">
              {Object.entries(currentRoleObj.permissions).map(([moduleName, actions]) => (
                <tr key={moduleName} className="hover:bg-white/5">
                  <td className="p-3.5 font-bold capitalize text-emerald-400">{moduleName}</td>
                  <td className="p-3.5">
                    <div className="flex flex-wrap gap-1.5">
                      {actions.map((act) => (
                        <span key={act} className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[11px]">
                          ✓ {act}
                        </span>
                      ))}
                    </div>
